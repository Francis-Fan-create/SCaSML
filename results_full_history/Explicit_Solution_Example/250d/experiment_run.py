...
#initialize wandb
wandb.init(project="Explicit_Solution_Example", notes="250 d", tags=["Normal Sphere test","Simple Uniform Test","Adam_LBFGS training"],mode="disabled") #debug mode
# wandb.init(project="Explicit_Solution_Example", notes="250 d", tags=["Normal Sphere test","Simple Uniform Test","Adam_LBFGS training"]) #working mode
...
if os.path.exists(r"results_full_history/Explicit_Solution_Example/250d/model_weights_Adam_LBFGS.params"):
    '''To Do: Retrain the model with new data points& Try new methods to reduce errors'''
    #load the model
    net=FNN([251]+[50]*5+[1],equation)
    net.load_state_dict(torch.load(r"results_full_history/Explicit_Solution_Example/250d/model_weights_Adam_LBFGS.params",map_location=device)) #the other indexes are left for external resources of weights
    trained_net=net
else:
    ...
    #train the model
    trained_model=optimizer.train(r"results_full_history/Explicit_Solution_Example/250d/model_weights_Adam_LBFGS.params")
