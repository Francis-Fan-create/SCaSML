...
#initialize wandb
wandb.init(project="Grad_Dependent_Nonlinear", notes="60 d", tags=["Adam training","L_inf_training"],mode="disabled") #debug mode
# wandb.init(project="Grad_Dependent_Nonlinear", notes="60 d", tags=["Adam training","L_inf_training"]) #working mode
wandb.config.update({"device": device}) # record device type

#initialize the equation
equation=Grad_Dependent_Nonlinear(n_input=61,n_output=1)
...
if os.path.exists(r"results_full_history/Grad_Dependent_Nonlinear/60d/model.ckpt-?"):
    ...
    #initialize the FNN
    #same layer width
    net1=dde.maps.jax.FNN([61]+[50]*5+[1], "tanh", "Glorot normal")
    net2=dde.maps.jax.FNN([61]+[50]*5+[1], "tanh", "Glorot normal")
    net3=dde.maps.jax.FNN([61]+[50]*5+[1], "tanh", "Glorot normal")    
