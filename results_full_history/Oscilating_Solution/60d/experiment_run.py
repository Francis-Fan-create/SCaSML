...
#initialize wandb
wandb.init(project="OScilating_Solution", notes="60 d", tags=["Adam training","L_inf_training"],mode="disabled") #debug mode
# wandb.init(project="OScilating_Solution", notes="60 d", tags=["Adam training","L_inf_training"]) #working mode
wandb.config.update({"device": device}) # record device type

#initialize the equation
equation=OScilating_Solution(n_input=61,n_output=1)
#check if trained model is already saved
if os.path.exists(r"results_full_history/OScilating_Solution/60d/model.ckpt-?"):
    ...
    model.restore(r"results_full_history/OScilating_Solution/60d/model.ckpt-?",verbose=1)
...
test2.test(r"results_full_history/OScilating_Solution/60d")
# #run the test for SimpleScaling
# test4=SimpleScaling(equation,solver1_3,solver2,solver3_3)
# test4.test(r"results_full_history/OScilating_Solution/60d")
