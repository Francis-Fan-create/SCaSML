...
#initialize wandb
wandb.init(project="Oscilating_Solution", notes="20 d", tags=["Adam training","L_inf_training"],mode="disabled") #debug mode
# wandb.init(project="Oscilating_Solution", notes="20 d", tags=["Adam training","L_inf_training"]) #working mode
wandb.config.update({"device": device}) # record device type

#initialize the equation
equation=Oscilating_Solution(n_input=21,n_output=1)
#check if trained model is already saved
if os.path.exists(r"results_full_history/Oscilating_Solution/20d/model.ckpt-?"):
    ...
    model.restore(r"results_full_history/Oscilating_Solution/20d/model.ckpt-?",verbose=1)
...
# test2.test(r"results_full_history/Oscilating_Solution/20d")
#run the test for SimpleScaling
test4=SimpleScaling(equation,solver1_3,solver2,solver3_3)
test4.test(r"results_full_history/Oscilating_Solution/20d")
