...
#initialize wandb
wandb.init(project="Linear_Convection_Diffusion", notes="10 d", tags=["Adam training","L_inf_training"],mode="disabled") #debug mode
# wandb.init(project="Linear_Convection_Diffusion", notes="10 d", tags=["Adam training","L_inf_training"]) #working mode
...
if os.path.exists(r"results_full_history/Linear_Convection_Diffusion/10d/model.ckpt-?"):
    ...
    #load the model
    net=dde.maps.jax.FNN([11]+[50]*5+[1], "tanh", "Glorot normal")
    ...
    model.restore(r"results_full_history/Linear_Convection_Diffusion/10d/model.ckpt-?",verbose=1)
...
test5.test(r"results_full_history/Linear_Convection_Diffusion/10d", budget_levels=[1.0,2.0,4.0])
