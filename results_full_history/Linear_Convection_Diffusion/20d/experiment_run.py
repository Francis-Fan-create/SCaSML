...
#initialize wandb
wandb.init(project="Linear_Convection_Diffusion", notes="20 d", tags=["Adam training","L_inf_training"],mode="disabled") #debug mode
# wandb.init(project="Linear_Convection_Diffusion", notes="20 d", tags=["Adam training","L_inf_training"]) #working mode
wandb.config.update({"device": device}) # record device type

#initialize the equation
equation=Linear_Convection_Diffusion(n_input=21,n_output=1)
#check if trained model is already saved
if os.path.exists(r"results_full_history/Linear_Convection_Diffusion/20d/model.ckpt-?"):
    ...
    model.restore(r"results_full_history/Linear_Convection_Diffusion/20d/model.ckpt-?",verbose=1)
...
test5.test(r"results_full_history/Linear_Convection_Diffusion/20d", budget_levels=[1.0,2.0,4.0])
