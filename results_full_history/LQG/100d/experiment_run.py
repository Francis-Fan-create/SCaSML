...
#initialize wandb
wandb.init(project="LQG", notes="100 d", tags=["Adam training","L_inf_training"],mode="disabled") #debug mode
# wandb.init(project="LQG", notes="100 d", tags=["Adam training","L_inf_training"]) #working mode
...
if os.path.exists(r"results_full_history/LQG/100d/model.ckpt-?"):
    '''To Do: Retrain the model with new data points& Try new methods to reduce errors'''
    #load the model
    net=dde.maps.jax.FNN([101]+[128]*4+[1], "tanh", "Glorot normal")
    ...
    model.restore(r"results_full_history/LQG/100d/model.ckpt-?",verbose=1)
    # set is_train to False
    is_train = False
else:
    #initialize the FNN
    #same layer width
    net1=dde.maps.jax.FNN([101]+[128]*4+[1], "tanh", "Glorot normal")
    net2=dde.maps.jax.FNN([101]+[128]*4+[1], "tanh", "Glorot normal")
    net3=dde.maps.jax.FNN([101]+[128]*4+[1], "tanh", "Glorot normal")    
    terminal_transform = equation.terminal_transform
