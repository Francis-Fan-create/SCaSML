...
#initialize wandb
wandb.init(project="LQG", notes="60 d", tags=["Adam training","L_inf_training"],mode="disabled") #debug mode
# wandb.init(project="LQG", notes="60 d", tags=["Adam training","L_inf_training"]) #working mode
wandb.config.update({"device": device}) # record device type

#initialize the equation
equation=LQG(n_input=61,n_output=1)
#check if trained model is already saved
if os.path.exists(r"results_full_history/LQG/60d/model.ckpt-?"):
    ...
    model.restore(r"results_full_history/LQG/60d/model.ckpt-?",verbose=1)
