...
#initialize wandb
wandb.init(project="Diffusion_Reaction", notes="80 d", tags=["Adam training","L_inf_training"],mode="disabled") #debug mode
# wandb.init(project="Diffusion_Reaction", notes="80 d", tags=["Adam training","L_inf_training"]) #working mode
wandb.config.update({"device": device}) # record device type

#initialize the equation
equation=Diffusion_Reaction(n_input=81,n_output=1)
#check if trained model is already saved
if os.path.exists(r"results_full_history/Diffusion_Reaction/80d/model.ckpt-?"):
    ...
    model.restore(r"results_full_history/Diffusion_Reaction/80d/model.ckpt-?",verbose=1)
    ...
    net3=dde.maps.jax.PFNN([[81]*5,[5]+[10]*3+[20],[5]+[10]*3+[20],[5]+[10]*3+[20],[5]+[10]*3+[20],[5]+[10]*3+[20],1], "tanh", "Glorot normal")    
