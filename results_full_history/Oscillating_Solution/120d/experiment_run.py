...
#initialize wandb
wandb.init(project="Oscillating_Solution", notes="120 d", tags=["Adam training","L_inf_training"],mode="disabled") #debug mode
# wandb.init(project="Oscillating_Solution", notes="120 d", tags=["Adam training","L_inf_training"]) #working mode
wandb.config.update({"device": device}) # record device type

#initialize the equation
equation=Oscillating_Solution(n_input=121,n_output=1)
#check if trained model is already saved
if os.path.exists(r"results_full_history/Oscillating_Solution/120d/model.ckpt-?"):
    '''To Do: Retrain the model with new data points& Try new methods to reduce errors'''
    #load the model
    net=dde.maps.jax.FNN([121]+[50]*5+[1], "tanh", "Glorot normal")
    ...
    model.restore(r"results_full_history/Oscillating_Solution/120d/model.ckpt-?",verbose=1)
    # set is_train to False
    is_train = False
else:
    #initialize the FNN
    #same layer width
    net1=dde.maps.jax.FNN([121]+[50]*5+[1], "tanh", "Glorot normal")
    net2=dde.maps.jax.FNN([121]+[50]*5+[1], "tanh", "Glorot normal")
    net3=dde.maps.jax.FNN([121]+[50]*5+[1], "tanh", "Glorot normal")    
...
test5.test(r"results_full_history/Oscillating_Solution/120d", budget_levels=[1.0,2.0,4.0])
