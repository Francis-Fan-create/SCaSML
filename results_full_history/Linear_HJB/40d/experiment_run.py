...
#initialize wandb
wandb.init(project="Linear_HJB", notes="40 d", tags=["Adam training","L_inf_training"],mode="disabled") #debug mode
# wandb.init(project="Linear_HJB", notes="40 d", tags=["Adam training","L_inf_training"]) #working mode
wandb.config.update({"device": device}) # record device type

#initialize the equation
equation=Linear_HJB(n_input=41,n_output=1)
#check if trained model is already saved
if os.path.exists(r"results_full_history/Linear_HJB/40d/model.ckpt-?"):
    ...
    model.restore(r"results_full_history/Linear_HJB/40d/model.ckpt-?",verbose=1)
