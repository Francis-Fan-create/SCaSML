...
#initialize wandb
wandb.init(project="Linear_HJB", notes="80 d", tags=["Adam training","L_inf_training"],mode="disabled") #debug mode
# wandb.init(project="Linear_HJB", notes="80 d", tags=["Adam training","L_inf_training"]) #working mode
wandb.config.update({"device": device}) # record device type

#initialize the equation
equation=Linear_HJB(n_input=81,n_output=1)
#check if trained model is already saved
if os.path.exists(r"results_full_history/Linear_HJB/80d/model.ckpt-?"):
    ...
    model.restore(r"results_full_history/Linear_HJB/80d/model.ckpt-?",verbose=1)
...
# rhomax=test1.test(r"results_full_history/Linear_HJB/80d")
