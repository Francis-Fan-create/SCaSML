...
class LQG(Equation):
    ...
    def PDE_loss(self, x_t,u,z):
        ...
        for k in range(self.n_input-1): # Accumulates laplacian and divergence over spatial dimensions.
            laplacian +=dde.grad.jacobian(z, x_t, i=k, j=k) # Computes the laplacian of z.
            square_div += dde.grad.jacobian(u, x_t, i=0, j=k)**2 # Computes the square of the divergence of u.
        residual=du_t-square_div+laplacian  # Computes the residual of the PDE.
        return residual 
    ...
    def gPDE_loss(self, x_t,u):
        ...
        for k in range(self.n_input-1): # Accumulates laplacian and divergence over spatial dimensions.
            laplacian +=dde.grad.hessian(u, x_t, i=k, j=k) # Computes the laplacian of u.
            square_div += dde.grad.jacobian(u, x_t, i=0, j=k) # Computes the divergence of u.
        residual=du_t-square_div+laplacian
        g_loss=[]
        for k in range(self.n_input-1): # Accumulates gradients of the residual for gPINN loss.
            g_loss.append(0.01*dde.grad.jacobian(residual,x_t,i=0,j=k)) # Computes gradient penalty.
        g_loss.append(residual) # Adds the residual to the loss.
    ...
    def terminal_constraint(self, x_t):
        ...
        '''
        x=x_t[:,:self.n_input-1] # Extracts the spatial coordinates.
        norm_x=np.linalg.norm(x,axis=1) # Computes the norm of spatial coordinates.
        result=np.log((1+norm_x)/2) # Computes the terminal constraint.
        return result 
    ...
    def f(self, x_t,u,z):
        ...
        for k in range(self.n_input-1):
            square_div += dde.grad.jacobian(u, x_t, i=0, j=k)**2
        result=-square_div/np.sqrt(2)
        return result
    ...
    def exact_solution(self, x_t):
        ...
        t = x_t[:, -1]
        MC=500
        brownian_step = np.sqrt(self.T-t)
        brownian_motion=np.random.normal(0,1,size=(x_t.shape[0],self.n_input-1,MC))
        x = x_t[:, :-1]
        repeat_x=np.repeat(x[:,:,np.newaxis],MC,axis=2)
        perturbed_x=repeat_x+brownian_step[:,np.newaxis,np.newaxis]*brownian_motion*np.sqrt(2)
        terminal_value=np.zeros((x_t.shape[0],1,MC))
        for i in range(MC):
            terminal_value[:,:,i]=self.terminal_constraint(perturbed_x[:,:,i])[:,np.newaxis]
        exp_value=np.exp(-terminal_value)
        result=-np.log(np.mean(exp_value,axis=2))
        return result
