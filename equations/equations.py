...
class Equation(object):
    ...
    def f(self, x_t,u,z):
        ...
        '''TO DO: why does setting z to be purely the gradient of u work?'''
...
class Explict_Solution_Example_Rescale(Equation):
    ...
    class LQG(Equation):
        ...
        def terminal_constraint(self, x_t):
            #notice that the result should be a 1d vector, with its rows being the batch size
            result= np.log((1+np.linalg.norm(x_t[:,:self.n_input-1])**2)/2)
            return result
        ...
        def f(self, x_t,u,z):
            #generator term for this PDE, returns a 2d vector
            result=-np.linalg.norm(z)**2
            return result
        
        def exact_solution(self, x_t):
            #exact solution of the example
            #simulate the solution: u(x_t)=-log(E[exp(-g(x+sqrt(2)*W_{T-t}))])
            #where W_{T-t} is a standard normal random variable
            ...
            disturbed_X = X + np.sqrt(2) *scale* W
            exp_value=np.exp(-self.g(disturbed_X))
            result=-np.log(np.mean(exp_value,axis=1,keepdims=True))
            return result
