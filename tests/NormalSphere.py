...
class NormalSphere(object):
    ...
    def test(self, save_path, rhomax=2, n_samples=100, x_grid_num=100, t_grid_num=10):
        ...
        print(f"PINN rel L2, rho={rhomax}->","min:",np.min(rel_error1),"max:",np.max(rel_error1),"mean:",np.mean(rel_error1))
        ...
        print(f"PINN L2, rho={rhomax}->","min:",np.min(errors1),"max:",np.max(errors1),"mean:",np.mean(errors1))
        ...
        print(f'PINN L2 - ScaSML L2,rho={rhomax}->','positve count:',np.sum(errors_13>0),'negative count:',np.sum(errors_13<0), 'positive sum:', positive_sum_13, 'negative sum:', negative_sum_13)
        ...
        print(f'PINN L2 - MLP L2,rho={rhomax}->','positve count:',np.sum(errors_12>0),'negative count:',np.sum(errors_12<0), 'positive sum:', postive_sum_12, 'negative sum:', negative_sum_12)
        ...
        wandb.log({f"positive count of PINN L2 - MLP L2,rho={rhomax}": np.sum(errors_12>0), f"negative count of PINN L2 - MLP L2,rho={rhomax}": np.sum(errors_12<0), f"positive sum of PINN L2 - MLP L2,rho={rhomax}": postive_sum_12, f"negative sum of PINN L2 - MLP L2,rho={rhomax}": negative_sum_12})
