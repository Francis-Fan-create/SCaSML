...
class NormalShperes(object):
    ...
    def test(self,savepath,rho=2,n_samples=10,x_grid_num=100,t_grid_num=10):
        ...
        wandb.log({"PINN error":errors1,"MLP error":errors2,"ScaML error":errors3})
        plt.figure()
        plt.contourf(x_mesh,t_mesh,errors1)
        plt.colorbar()
        plt.title("PINN error")
        plt.xlabel("distance from origin")
        plt.ylabel("time")
        plt.savefig("PINN_error.png")
        plt.figure()
        plt.contourf(x_mesh,t_mesh,errors2)
        plt.colorbar()
        plt.title("MLP error")
        plt.xlabel("distance from origin")
        plt.ylabel("time")
        plt.savefig("MLP_error.png")
        plt.figure()
        plt.contourf(x_mesh,t_mesh,errors3)
        plt.colorbar()
        plt.title("ScaML error")
        plt.xlabel("distance from origin")
        plt.ylabel("time")
        plt.savefig("ScaML_error.png")
