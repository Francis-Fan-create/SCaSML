...
class NormalShperes(object):
    ...
    def test(self,rho=2,n_samples=10,x_grid_num=100,t_grid_num=10):
        ...
        for i in range(x_mesh.shape[0]):
            for j in range(x_mesh.shape[1]):
                ...
                sol2=self.solver2.uz_solve(n,rho,np.concatenate((x,[t_mesh[i,j]]))[np.newaxis,:])[:,0]
                sol3=self.solver3.u_solve(n,rho,np.concatenate((x,[t_mesh[i,j]]))[np.newaxis,:])
                errors1_sum+=np.mean(np.linalg.norm(sol1-exact_sol,axis=1))
                errors2_sum+=np.mean(np.linalg.norm(sol2-exact_sol,axis=1))
                errors3_sum+=np.mean(np.linalg.norm(sol3-exact_sol,axis=1))
                errors
