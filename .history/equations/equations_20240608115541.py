...
class Equation(object):
    ...
    def terminal_condition(self,x_t):
        ...
        if hasattr(self, 'terminal_constraint') and hasattr(self, 'geometry'):
            tc=dde.OperatorBC(self.geometry(),self.terminal_constraint, lambda _, on_boundary: on_boundary and np.isclose(x_t[:,-1],self.T)) #need to be enforced on generate_data method
            return tc
        else:
            raise NotImplementedError
    def boundary_condition(self):
        ...
        if hasattr(self, 'boundary_constraint') and hasattr(self, 'geometry'):
            bc=dde.icbc.DirichletBC(self.geometry(),self.boundary_constraint, lambda _, on_boundary: on_boundary) #need to be enforced on generate_data meth
            return bc
        else:
            raise NotImplementedError
...
class Explict_Solution_Example(Equation):
    ...
    def generate_data(self, num_domain=1000,num_boundary=200):
        data = dde.data.TimePDE( #time dependent PDE
                                ...
                                num_boundary=num_boundary, #sample how many points on the boundary
