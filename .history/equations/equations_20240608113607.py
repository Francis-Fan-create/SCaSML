...
class Equation(object):
    ...
    def terminal_condition(self,x_t,u):
        ...
        if hasattr(self, 'terminal_constraint') and hasattr(self, 'geometry'):
            tc=dde.OperatorBC(self.geometry(),self.terminal_constraint, lambda _, on_boundary: on_boundary and np.isclose(x[-1])) #need to be enforced on generate_data method
        else:
            raise NotImplementedError
