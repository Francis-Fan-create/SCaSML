...
class Equation(object):
    ...
    def terminal_condition(self):
        ...
        if hasattr(self, 'terminal_constraint') and hasattr(self, 'geometry'):
            ic=dde.icbc.IC(self.geometry(),self.terminal_constraint, lambda _, on_terminal: on_terminal)
            self.terminal_condition=ic
            return ic
        else:
            raise NotImplementedError
...
class Explict_Solution_Example(Equation):
    ...
    def PDE_loss(self, x_t,u):
        ...
        return g_loss
    def terminal_constraint(self, x_t):
        result=np.exp(x_t[:,-1] + np.sum(x_t[0:self.n_input],axis=1)) / (1 + np.exp(x_t[:,-1] + np.sum(x_t[],axis=1)))
        return result
    ...
    def generate_data(self, num_domain=7000, num_terminal=40,num_test=10):
        data = dde.data.TimePDE( #time dependent PDE
                                ...
                                [self.terminal_condition], #terminal condition
                                num_domain=num_domain, #sample how many points in the domain
                                num_boundary=0, #sample how many points on the boundary
                                num_terminal=num_terminal,  #sample how many points for the terminal time
