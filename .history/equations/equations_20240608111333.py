...
class Equation(object):
    ...
    def terminal_condition(self,x_t,u):
        ...
        if hasattr(self, 'terminal_constraint') and hasattr(self, 'geometry'):
            x_t = x_t.detach().numpy() if isinstance(x_t, torch.Tensor) else x_t
            u = u.detach().numpy() if isinstance(u, torch.Tensor) else u
...
class Explict_Solution_Example(Equation):
    ...
    def gPDE_loss(self, x_t,u):
        ...
        return g_loss
    def terminal_constraint(self, x_t):
        result=np.exp(x_t[:,-1] + np.sum(x_t[:,:self.n_input],axis=1)) / (1 + np.exp(x_t[:,-1] + np.sum(x_t[:,0:self.n_input],axis=1)))
        return result
