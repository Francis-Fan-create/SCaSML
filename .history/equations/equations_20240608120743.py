...
class Equation(object):
    ...
    def terminal_condition(self):
        ...
        if hasattr(self, 'terminal_constraint') and hasattr(self, 'geometry'):
            tc=dde.DirichletBC(self.geometry(),self.terminal_constraint, lambda x_t, on_boundary: on_boundary and np.isclose(x_t[])) #need to be enforced on generate_data method
            return tc
        else:
            raise NotImplementedError
