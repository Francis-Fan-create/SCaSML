...
class Equation(object):
    ...
    def terminal_condition(self,x_t,u):
        ...
        if hasattr(self, 'terminal_constraint') and hasattr(self, 'geometry'):
            x_t = torch.tensor(x_t,requires_grad=True) if not isinstance(x_t, torch.Tensor) else x_t
            u = torch.tensor(u,requires_grad=True) if not isinstance(u, torch.Tensor) else u
            result=(x_t[:,-1] - self.T)*u+self.terminal_constraint(x_t)
            return torch.tensor(,requires_grad=True) #need to be enforced on network class
        else:
            raise NotImplementedError
...
class Explict_Solution_Example(Equation):
    ...
    def generate_data(self, num_domain=1000,num_test=None):
