...
class Adam_LBFGS(object):
    ...
    def train(self,save_path,cycle=40,adam_every=500,lbfgs_every=10,metrics=["l2 relative error","mse"]):
        ...
        loss_weights=torch.softmax(torch.tensorself.model.train_state.loss_train).detach().tolist()                           #use self adaptive loss weights
        ...
        for i in range(cycle):
            ...
            wandb.log({"Adam loss": self.model._outputs_losses(), "Adam metrics": self.model.metrics})  # record loss and metrics
            ...
            wandb.log({"LBFGS loss": self.model._outputs_losses(), "LBFGS metrics": self.model.metrics})  # record loss and metrics
