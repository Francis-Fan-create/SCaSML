...
class Adam_LBFGS(object):
    ...
    def train(self,save_path,cycle=40,adam_every=500,lbfgs_every=10,metrics=["l2 relative error","mse"]):
        ...
        loss_weights=[1e-3]*range(self.n_input)+[1]+[1e-2]
        wandb.config.update({"cycle": cycle, "adam_every": adam_every, "lbfgs_every": lbfgs_every,"loss_weights":lo}) # record hyperparameters
