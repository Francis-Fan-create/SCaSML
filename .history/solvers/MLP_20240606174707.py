...
class MLP(object):
    ...
    def uz_solve(self,n, rho, x_t): 
        ...
        X = np.repeat(x[:, np.newaxis, :], MC, axis=1) + sigma * W
        u = np.mean(np.apply_along_axis(g, 1, X), axis=1)
        z = np.sum((np.repeat(np.apply_along_axis(g, 1, X)-g(x)[:, np.newaxis] * np.ones((batch_size,MC, 1)), dim, axis=-1) * W), axis=1) / (MC * (T-t)[:, np.newaxis])
        if n <= 0:
            return np.concatenate((u, z),axis)
        for l in range(n):
            q = int(Q[rho-1, n-l-1])
