...
class MLP(object):
    ...
    def inverse_gamma(self, gamma_input):
        ...
        c = 0.036534 # Euler's constant
        L = np.log((gamma_input+c) / np.sqrt(2 * np.pi)) # Lambert W function
    ...
    def lgwt(self,N, a, b):
        ...
        y = np.cos((2 * np.arange(0, N+1, 1)+ 1) * np.pi / (2 * N + 2))+(0.27/N1) * np.sin(np.pi * xu * N / N2) # Chebyshev-Gauss nodes
        L = np.zeros((N1, N2)) # Legendre polynomial
        Lp = np.zeros((N1, N2)) # derivative of Legendre polynomial
        y0 = 2 # initial guess
        while np.max(np.abs(y-y0)) > 2.2204e-16: # tolerance
            L[:, 0] = 1 
            Lp[:, 0] = 0
            L[:, 1] = y 
            Lp[:, 1] = 1
            ...
            y0 = y
            y = y0 - L[:, N2-1] / Lp
        x = (a * (1-y) + b * (1+y)) / 2 # nodes
        w = (b-a) / ((1-y*y) * Lp * Lp) * N2 * N2 / (N1 * N1) # weights
        return x[0], w[0]

    def approxparameters(self,rhomax):
        #approximate parameters for the MLP
        n = list(range(1, rhomax+1)) #level list
