...
class MLP(object):
    ...
    def uz_solve(self,n, rho, x_t): 
        ...
        cloc = (T-t).unsqueeze(-1) * c/T #local time
        wloc = (T-t).unsqueeze(-1) * w/T #local weights
        MC = int(Mg[rho-1, n]) # number of monte carlo samples for backward Euler
        W = np.sqrt(T-t).unsqueeze(-1) * np.random.normal(size=(batch_size,MC, dim))
        X = np.repeat([x], MC, axis=1) + sigma * W
        u = np.mean(np.repeat(np.apply_along_axis(g, 1, X), axis=1)
        z = np.nansum(np.repeat(np.apply_along_axis(g, 1, X)-g(x) * np.ones((MC, 1)), dim, axis=1) * W, axis=0) / (MC * (T-s))
        if n <= 0:
        ...
        for l in range(n):
            ...
            for k in range(q):
                ...
                W += dW           
                X += sigma * dW            
                mat = np.array([self.uz_solve(l, rho, X[i], cloc[k, q-1]) for i in range(MC)])
