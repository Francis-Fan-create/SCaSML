...
class MLP(object):
    ...
    def uz_solve(self,n, rho, x_t): 
        ...
        cloc = (T-t)[:, np.newaxis,np.newaxis] * c[]/T #local time
        wloc = (T-t)[:, np.newaxis,np.newaxis] * w/T #local weights
        ...
        for l in range(n):
            ...
            for k in range(q):
                ...
                y = [f(cloc[:,k,q-1], X[:,i,:], simulated_u[:,i,:], simulated_z[:,i,:]) for i in range(MC)]
