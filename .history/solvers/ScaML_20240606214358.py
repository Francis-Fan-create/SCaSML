...
class ScaML(object):
    ...
    def f(self,x_t,u,z):
        ...
        grad_u_hat=torch.autograd.grad(u_hat,tensor_x_t,grad_outputs=torch.ones_like(u_hat),create_graph=True)[0][:, :-1].detach().numpy()
        epsilon=self.approx_PDE_loss(x_t,u_hat,grad_u_hat)
