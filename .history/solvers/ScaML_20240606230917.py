...
class ScaML(object):
    ...
    def f(self,x_t,u_breve,z):
        ...
        val1=eq.f(x_t,u_breve+u_hat,z+eq.sigma(x_t)*grad_u_hat_x)  
    ...
    def u_breve_z_solve(self,n, rho, x_t): 
        #approximate the solution of the PDE, return the value of u_breve(x_t) and z(x_t), batchwisely
        ...
        if n <= 0:
            return np.concatenate((u_breve, z),axis=-1)
        for l in range(n):
            ...
            for k in range(q):
                ...
                W += dW           
                X += sigma * dW            
                simulated=np.apply_along_axis(lambda x_t:self.u_breve_z_solve(n=l,rho=rho,x_t=x_t),1,np.concatenate((X, np.repeat(cloc[:,k,q-1],MC,axis=1)),axis=-1))
                simulated_u_breve, simulated_z = simulated[:,:, 0].reshape(batch_size,MC,1), simulated[:,:, 1:] 
                y = np.array( [f(cloc[:,k,q-1], X[:,i,:], simulated_u_breve[:,i,:], simulated_z[:,i,:]) for i in range(MC)])
                y=y.transpose(1,0,2)
                u_breve += wloc[:,k, q-1] * np.mean(y, axis=1) 
                z += wloc[:,k, q-1] * np.sum(np.repeat(y[:,:,0], dim, axis=-1) * W, axis=1) / (MC * (cloc[:,k, q-1]-t)[:,np.newaxis])
                if l:
                    simulated=np.apply_along_axis(lambda x_t:self.u_breve_z_solve(n=l-1,rho=rho,x_t=x_t),1,np.concatenate((X, np.repeat(cloc[:,k,q-1],MC,axis=1)),axis=-1))
                    simulated_u_breve, simulated_z = simulated[:,:, 0].reshape(batch_size,MC,1), simulated[:,:, 1:] 
                    y = np.array( [f(cloc[:,k,q-1], X[:,i,:], simulated_u_breve[:,i,:], simulated_z[:,i,:]) for i in range(MC)])
                    y=y.transpose(1,0,2)
                    u_breve -= wloc[:,k, q-1] * np.mean(y, axis=1) 
    ...
    def uz_solve(self,n, rho, x_t): 
        ...
        u_breve_z=self.u_breve_z_solve(n, rho, x_t)
        u_breve, z = u_breve_z[:, 0].reshape(-1,1), u_breve_z[:, 1:]
        eq=self.equation
        tensor_x_t=torch.tensor(x_t,requires_grad=True).float()
        u_hat=self.net(tensor_x_t).detach().numpy()
        grad_u_hat_x=torch.autograd.grad(u_hat,tensor_x_t,grad_outputs=torch.ones_like(u_hat),create_graph=True)[0][:, :-1].detach().numpy()
