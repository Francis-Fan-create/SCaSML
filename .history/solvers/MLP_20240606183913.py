    ...
    def uz_solve(self,n, rho, x_t): 
        ...
        for l in range(n):
            ...
            for k in range(q):
                ...
                W += dW           
                X += sigma * dW            
                mat = np.array([self.uz_solve(l, rho, np.concatenate((X[i], cloc[k, q-1]),axis)) for i in range(MC)])
                simulated=np.apply_along_axis(lambda x_t:self.uz_solve(n=l,rho=rho,x_t=x_t),1)
