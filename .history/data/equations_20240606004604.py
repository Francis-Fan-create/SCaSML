...
class Explict_Solution_Example(Equation):
    ...
    def PDE_Loss(self, x_t,u):
        ...
        laplacian=0
        div=0
        for k in range(self.n_input): #here, we use a slower accumulating method to avoid computing more autograd, which is a tradeoff
            laplacian +=dde.grad.hessian(u, x_t, i=k, j=k) #lazy
            div += dde.grad.jacobian(u, x_t, i=0, j=k) #lazy
        pde_loss=du_t + (self.sigma**2 * u - 1/self.n_input - self.sigma**2/2) * div + sigma**2/2 * laplacian
        g_loss=[]
        for k in range(d):
