...
class Adam_LBFGS(object):
    ...
    def train(self,cycle=20,adam_every=1000,lbfgs_every=10,metrics=["l2 relative error","mse"]):
        #interleaved training of adam and lbfgs
        every_interleave = adam_every + lbfgs_every
        for i in range(cycle):
            self.model.compile(optimizer=self.Adam(),metrics=metrics)
            self.model.train(epochs=adam_every, display_every=100, metrics=metrics)
            self.model.compile("lbfgs")
            self.model.train(epochs=lbfgs_every, display_every=100, metrics=metrics)
        return self.model