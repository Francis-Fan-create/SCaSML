...
if os.path.exists(r"results/Complicated_HJB/250d/model_weights_Adam_LBFGS_L_inf.params"):
    '''To Do: Retrain the model with new data points& Try new methods to reduce errors'''
    #load the model
    net=FNN([251]+[50]*5+[1],equation)
    net.load_state_dict(torch.load(r"results/Complicated_HJB/250d/model_weights_Adam_LBFGS_L_inf.params",map_location=device)) #the other indexes are left for external resources of weights
    trained_net=net
else:
    ...
    net.load_state_dict(torch.load(r"results/Complicated_HJB/250d/model_weights_Adam_LBFGS.params",map_location=device)) #the other indexes are left for external resources of weights
    #initialize the optimizer
    # optimizer=Adam_LBFGS(251,1,net,data) #Adam-LBFGS optimizer
    optimizer=L_inf(251,1,net,data,equation) #L_inf optimizer
    #train the model
    trained_model=optimizer.train(r"results/Complicated_HJB/250d/model_weights_Adam_LBFGS_L_inf.params")
