# We will use this file to run the experiment for the Complicated_HJB equation. We will train the model and then test it on the NormalSphere test. We will also profile the test to see the time taken by each solver to solve the equation. We will save the profiler results and upload them to wandb.
import sys
import os
...
#initialize wandb
wandb.init(project="Complicated_HJB", notes="100 d", tags=["normal sphere test","L_inf training"],mode="disabled") #debug mode
# wandb.init(project="Complicated_HJB", notes="100 d", tags=["normal sphere test","L_inf training"]) #working mode
wandb.config.update({"device": device.type}) # record device type

#initialize the equation
equation=Complicated_HJB(n_input=101,n_output=1)
...
if os.path.exists(r"results/Complicated_HJB/model_weights_1.params"):
    ...
    #train the model
    trained_model=optimizer.train("results/Complicated_HJB/model_weights_1.params")
    # trained_model=optimizer.train("results/Complicated_HJB/model_weights_2.params")   
...
rhomax=test.test(r"results/Complicated_HJB")
#stop the profiler
profiler.disable()
#save the profiler results
profiler.dump_stats(f"results/Complicated_HJB/Complicated_HJB_profiler_rho_{rhomax}.prof")
#upload the profiler results to wandb
artifact=wandb.Artifact(f"Complicated_HJB_profiler_rho_{rhomax}", type="profile")
artifact.add_file(f"results/Complicated_HJB/Complicated_HJB_profiler_rho_{rhomax}.prof")
