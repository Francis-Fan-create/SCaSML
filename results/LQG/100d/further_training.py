...
#initialize wandb
wandb.init(project="LQG", notes="100 d", tags=["Normal Sphere test","Simple Uniform Test","L_inf training"],mode="disabled") #debug mode
# wandb.init(project="LQG", notes="100 d", tags=["Normal Sphere test","Simple Uniform Test","Adam_LBFGS training"],mode="disabled") #debug mode
# wandb.init(project="LQG", notes="100 d", tags=["Normal Sphere test","Simple Uniform Test","L_inf training"]) #working mode
# wandb.init(project="LQG", notes="100 d", tags=["Normal Sphere test","Simple Uniform Test","Adam_LBFGS training"]) #working mode
...
if os.path.exists(r"results/LQG/100d/model_weights_Adam_LBFGS_L_inf.params"):
    '''To Do: Retrain the model with new data points& Try new methods to reduce errors'''
    #load the model
    net=FNN([101]+[50]*5+[1],equation)
    net.load_state_dict(torch.load(r"results/LQG/100d/model_weights_Adam_LBFGS_L_inf.params",map_location=device)) #the other indexes are left for external resources of weights
    trained_net=net
else:
    ...
    net.load_state_dict(torch.load(r"results/LQG/100d/model_weights_Adam_LBFGS.params",map_location=device)) #the other indexes are left for external resources of weights
    #initialize the optimizer
    # optimizer=Adam_LBFGS(101,1,net,data) #Adam-LBFGS optimizer
    optimizer=L_inf(101,1,net,data,equation) #L_inf optimizer
    #train the model
    trained_model=optimizer.train(r"results/LQG/100d/model_weights_Adam_LBFGS_L_inf.params")
...
rhomax=test1.test(r"results/LQG/100d")
#run the test for SimpleUniform
test2=SimpleUniform(equation,solver1,solver2,solver3)
test2.test(r"results/LQG/100d")
