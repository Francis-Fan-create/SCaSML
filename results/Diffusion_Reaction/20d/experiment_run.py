...
#initialize wandb
wandb.init(project="Diffusion_Reaction", notes="20 d", tags=["Adam training","L_inf_training"],mode="disabled") #debug mode
# wandb.init(project="Diffusion_Reaction", notes="20 d", tags=["Adam training","L_inf_training"]) #working mode
wandb.config.update({"device": device}) # record device type

#initialize the equation
equation=Diffusion_Reaction(n_input=21,n_output=1)
#check if trained model is already saved
if os.path.exists(r"results/Diffusion_Reaction/80d/model.ckpt-?"):
    '''To Do: Retrain the model with new data points& Try new methods to reduce errors'''
    #load the model
    net=dde.maps.jax.PFNN([[21]*5,[5]+[10]*3+[20],[5]+[10]*3+[20],[5]+[10]*3+[20],[5]+[10]*3+[20],[5]+[10]*3+[20],1], "tanh", "Glorot normal")
    ...
    model.restore(r"results/Diffusion_Reaction/20d/model.ckpt-?",verbose=1)
    # set is_train to False
    is_train = False
else:
    #initialize the FNN
    #same layer width
    net1=dde.maps.jax.PFNN([[21]*5,[5]+[10]*3+[20],[5]+[10]*3+[20],[5]+[10]*3+[20],[5]+[10]*3+[20],[5]+[10]*3+[20],1], "tanh", "Glorot normal")
    net2=dde.maps.jax.PFNN([[21]*5,[5]+[10]*3+[20],[5]+[10]*3+[20],[5]+[10]*3+[20],[5]+[10]*3+[20],[5]+[10]*3+[20],1], "tanh", "Glorot normal")
    net3=dde.maps.jax.PFNN([[21]*5,[5]+[10]*3+[20],[5]+[10]*3+[20],[5]+[10]*3+[20],[5]+[10]*3+[20],[5]+[10]*3+[20],1], "tanh", "Glorot normal")    
