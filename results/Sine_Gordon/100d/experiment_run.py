...
if os.path.exists(r"results/Sine_Gordon/100d/model_weights_L_inf.params"):
    '''To Do: Retrain the model with new data points& Try new methods to reduce errors'''
    #load the model
    net=FNN([101]+[50]*5+[1],equation)
    net.load_state_dict(torch.load(r"results/Sine_Gordon/100d/model_weights_L_inf.params",map_location=device)) #the other indexes are left for external resources of weights
    trained_net=net
else:
    ...
    #train the model
    trained_model=optimizer.train(r"results/Sine_Gordon/100d/model_weights_L_inf.params")
